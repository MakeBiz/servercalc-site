...
export function providersForTask(taskSlug, locale = 'ru') {
  return providersFor(locale).filter((p) => (p.tasks || []).includes(taskSlug));
}

/* ---------- локаль: английская версия прячет Россию и провайдеров с русским сайтом ---------- */

/** Провайдеры под локаль: английская прячет тех, у кого enHidden (сайт только на русском) */
export function providersFor(locale) {
  return locale === 'en' ? PROVIDERS.filter((p) => !p.enHidden) : PROVIDERS;
}

/** Гео-страницы под локаль: английская без России */
export function geoPagesFor(locale) {
  return locale === 'en' ? GEO_PAGES.filter((g) => g.code !== 'ru') : GEO_PAGES;
}

/** Тарифы под локаль: английская без гео Россия и без тарифов скрытых провайдеров */
export function visiblePlansFor(locale) {
  if (locale !== 'en') return VISIBLE_PLANS;
  const hidden = new Set(PROVIDERS.filter((p) => p.enHidden).map((p) => p.slug));
  return VISIBLE_PLANS.filter((p) => p.geo !== 'ru' && !hidden.has(p.providerSlug));
}

export function providersForGeo(geoCode, locale = 'ru') {
  return providersFor(locale).filter((p) => (p.geos || []).includes(geoCode));
}

export function plansForGeo(geoCode, locale = 'ru') {
  return visiblePlansFor(locale).filter((p) => p.geo === geoCode).sort((a, b) => a.priceRub - b.priceRub);
}
...
export function catalogRows(locale = 'ru') {
  return visiblePlansFor(locale).map((plan) => {
    ...
    return {
      ...
      geoNameEn: getGeoByCode(plan.geo)?.nameEn || getGeoByCode(plan.geo)?.name || plan.geo,
...
export function calculatorPayload(locale = 'ru') {
  return {
    providers: providersFor(locale).map((p) => ({
      slug: p.slug,
      name: p.name,
      color: p.color,
      country: p.country,
    ...
    plans: visiblePlansFor(locale).map((p) => ({
      id: p.id,
