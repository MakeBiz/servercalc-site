...
export const METRIKA_ID = (() => {
  ...
  if (v === undefined) return site.metrikaId || null;   // servercalc.online → 111369260
  if (v && v !== '0') return v;                          // явно задан номер в окружении
  // На serverselection.online переменная задана пустой (Метрика раньше была выключена).
  // Включаем свой счётчик 111700664 прямо в коде — переменные окружения Vercel править не из чего.
  // На остальных доменах пустая переменная по-прежнему означает «Метрика выключена».
  if (/serverselection/.test(SITE_URL)) return '111700664';
  return null;
})();
