...
import { USD_RATE } from './format';

// Казахстан отнесён к Азии: выбор «Азия» в калькуляторе показывает и казахстанские
// площадки. Разворачивается только для asia, поэтому страница /vps-in/kazahstan
// продолжает матчиться отдельно по коду kz
const GEO_ALIASES = { asia: ['asia', 'kz'] };
function geoCodesFor(code) {
  return code ? GEO_ALIASES[code] || [code] : null;
}
...
// Редакционное закрепление порядка: процент соответствия НЕ меняется, комиссия в
// формулу не входит. На иностранной версии (locale === 'en') РФ-провайдеры наверх
// не закрепляются — они идут по баллу как обычные строки (см. runMatch)
export const PINNED = ['ishosting', 'timeweb', 'adminvps', 'hostman', 'beget'];
export const PIN_FIT = 50; // минимальный балл, чтобы закреплять
...
export const WEIGHTS_TABLE_EN = [
  { label: 'Base score', value: '42', note: 'Every provider starts here' },
  { label: 'Workload match', value: '+25 / −12', note: 'The provider lists the scenario or not. No workload chosen: +6' },
  { label: 'Location', value: '+22 / −26', note: 'Has a location in the region you need. Location not important: +6' },
  { label: 'Requirements', value: '+20 / −24', note: 'Proportional to the share of requirements met. Nothing selected: +6' },
  { label: 'Resources covered', value: '+8 / −10', note: 'A plan at or above the requested cores and memory exists' },
  { label: 'Within budget', value: '+10 / −10', note: 'The matching plan’s price fits the limit' },
  { label: 'Price per resource', value: '+6 … −6', note: 'Price per GB of memory relative to the base median' },
  { label: 'Total', value: '6 … 99', note: 'Clamped to a range; zero and one hundred are never given' },
];
 ...
 */
function pickPlan(plans, query, geoCodes) {
  if (!plans.length) return null;
  const wantNvme = (query.requirements || []).includes('nvme');
  const byPrice = (a, b) => a.priceRub - b.priceRub;

  const inGeo = geoCodes ? plans.filter((p) => geoCodes.includes(p.geo)) : plans;
...
export function scoreProvider(provider, plans, query, dict = {}) {
  ...
  const en = locale === 'en';
  const nm = (obj) => (en ? obj?.nameEn || obj?.name : obj?.name);
  const geo = query.geo && query.geo !== 'any' ? dict.geos?.find((g) => g.slug === query.geo) : null;
  const geoCode = geo?.code || null;
  const geoCodes = geoCodesFor(geoCode);
  const geoName = geo ? nm(geo) : '';
  const plan = pickPlan(plans, query, geoCodes);
  const reasons = [];
  ...
  if (query.task) {
    ...
    const taskName = nm(task) || query.task;
    ...
    reasons.push({
      ...
      ok,
      text: en
        ? ok ? `Listed for “${taskName}”` : `“${taskName}” is not listed by the provider`
        : ok ? `Заявлен под задачу «${taskName}»` : `Задача «${taskName}» не заявлена провайдером`,
    });
  } else {
  ...
  // 2. география
  if (geoCode) {
    const ok = (provider.geos || []).some((g) => geoCodes.includes(g));
    score += ok ? WEIGHTS.geo.match : WEIGHTS.geo.miss;
    reasons.push({
      code: 'geo',
      ok,
      text: en
        ? ok ? `Has a location: ${geoName}` : `No location in ${geoName}`
        : ok ? `Есть локация: ${geoName}` : `Нет локации в регионе ${geoName}`,
    });
  } else {
  ...
  if (reqs.length) {
    ...
    const nameOf = (c) => {
      const r = dict.requirements?.find((x) => x.code === c);
      return (en ? r?.shortEn || r?.short : r?.short) || c;
    };
    reasons.push({
      code: 'requirements',
      ok: missing.length === 0,
      text: en
        ? missing.length === 0
          ? `Meets all requirements: ${reqs.map(nameOf).join(', ')}`
  ...
  reasons.push({
    ...
    text: en
      ? plan
        ? covers
          ? `Plan covers ${query.cpu} ${query.cpu === 1 ? 'core' : 'cores'} and ${query.ram} GB RAM`
          : `Closest plan is below your request: ${plan.cpu} × ${plan.ram} GB`
        : 'No matching plans in the base'
      : plan
        ? covers
  ...
  if (query.budget && plan) {
    ...
    const overRub = Math.round(plan.priceRub - query.budget);
    reasons.push({
      code: 'budget',
      ok,
      text: en
        ? ok ? 'Within budget' : `Over budget by $${Math.max(1, Math.round(overRub / USD_RATE))}`
        : ok ? 'Укладывается в бюджет' : `Дороже лимита на ${overRub} ₽`,
    });
  }
  ...
  if (plan && dict.medianPerGb && plan.ram > 0) {
    ...
    score += delta;
    // Пометку про цену за гигабайт памяти пользователю НЕ показываем: этот параметр
    // он в фильтрах не задаёт, а на топовых результатах «дороже медианы» читалось
    // как проблема. На балл (тай-брейк против одинаковых оценок) она влияет по-прежнему
  }
...
export function runMatch(payload, query, locale = 'ru') {
  const dict = {
    tasks: payload.tasks,
    geos: payload.geos,
  ...
  // Закрепление приоритетных наверх. На иностранной версии РФ-провайдеров не закрепляем.
  const intl = locale === 'en';
  const pinRank = (r) => {
    const i = PINNED.indexOf(r.provider.slug);
    if (i === -1 || r.score < PIN_FIT) return Infinity;       // не в списке или нерелевантен запросу
    if (intl && r.provider.country === 'RU') return Infinity; // иностранная версия: РФ не закрепляем
    return i;
  };
  return scored
