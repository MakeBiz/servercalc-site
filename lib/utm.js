...
export function withUtm(rawUrl, { campaign, content, medium = 'referral' } = {}) {
  if (!rawUrl) return null;
  try {
    const url = new URL(rawUrl);
    url.searchParams.set('utm_source', SITE.utmSource);
    // utm_medium несёт язык версии: ru для русских страниц, eng для английских.
    // Так в кабинете провайдера видно, с какой языковой версии пришёл переход.
    // utm_campaign несёт площадку/страницу, utm_content несёт провайдера
 ...
 */
export function providerLink(provider, { campaign, content, medium } = {}) {
  ...
  const href = provider.noUtm
    ? provider.url
    : withUtm(provider.url, { campaign, content: content || provider.slug, medium });
  return {
    href,
