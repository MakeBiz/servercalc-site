...
export async function generateMetadata({ params }) {
  ...
  return {
    ...
    alternates: { canonical: `/ru/vps/${slug}` },
  };
}
  ...
  return (
    <>
      ...
      <PageHead
        ...
        lead={geo.note}
        crumbs={[
          { href: '/ru/vps', label: 'География' },
          { href: `/ru/vps/${slug}`, label: geo.name },
        ]}
        badges={
          <>
        ...
        <div className="wrap">
          ...
          <div className="chips">
            {GEO_PAGES.filter((g) => g.slug !== slug).map((g) => (
              <Link key={g.slug} href={`/ru/vps/${g.slug}`} className="chip chip-light">
                {g.name}
              </Link>
            ))}
          </div>
